...
class PosOrder(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            product_ids = []
            for command in vals.get('lines', []):
                if len(command) >= 3 and isinstance(command[2], dict) and command[2].get('product_id'):
                    product_ids.append(command[2]['product_id'])
            if product_ids:
                prepare_times = self.env['product.product'].search(
                    [('id', 'in', product_ids)]).mapped(
                    'prepair_time_minutes')
                vals['avg_prepare_time'] = max(prepare_times or [0.0])
    ...
    def _get_kot_report_lines(self):
        """Return the lines that should appear on the demo KOT report."""
        self.ensure_one()
        return self.lines.filtered(lambda line: line.qty > 0)

    def _get_kot_line_note(self, line):
        return (
            getattr(line, 'customer_note', False)
            or getattr(line, 'note', False)
            or getattr(line, 'order_note', False)
            or getattr(line, 'product_note', False)
            or ''
        )

    def _get_kot_product_name(self, line):
        """Return a safe display name for a KOT line's product.

        Computed in Python to avoid a QWeb compilation issue where chained
        dotted attribute access inside t-out raises KeyError: 'getattr' under
        the report rendering namespace.
        """
    ...
    def _get_kot_cashier_name(self):
        """Return the server / cashier name printed on the KOT."""
        self.ensure_one()
        employee = getattr(self, 'employee_id', False)
        if employee:
            return employee.name or ''
        user = getattr(self, 'user_id', False)
        if user:
            return user.name or ''
        return ''

    def _get_kot_guest_count(self):
        """Return guest/covers count when pos_restaurant exposes it, else 0."""
        self.ensure_one()
        try:
            value = int(getattr(self, 'customer_count', 0) or 0)
        except (TypeError, ValueError):
            value = 0
        return value

    def _get_kot_print_time(self):
        """Return the current print timestamp formatted in user's timezone."""
        self.ensure_one()
        now_utc = fields.Datetime.now()
        printed = fields.Datetime.context_timestamp(self, now_utc)
        return printed.strftime('%d-%b-%Y %H:%M:%S')
    ...
    @api.model
    def get_kot_pdf_report_url(self, pos_reference, config_id):
        """Return a direct PDF report URL for the POS order KOT preview."""
        _logger.info(
            "get_kot_pdf_report_url called with pos_reference=%s, config_id=%s",
            pos_reference,
            config_id,
        )
        pos_order = self.search([
        ...
        if not pos_order:
            raise UserError("POS order not found for KOT PDF preview.")
        report_name = 'pos_kitchen_screen_odoo.report_pos_kot_pdf'
        return f"/report/pdf/{report_name}/{pos_order.id}?download=true"
